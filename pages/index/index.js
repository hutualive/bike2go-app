...
// 页面加载
  onLoad: function (options) {
        ...
        else
        {
          that.setData({
            ...
            [
              ...
              {
                id: 1,
                iconPath: '/images/bleConnected.png',
                position: {
                  left: 20,
                  top: res.windowHeight - 75,
                  width: 50,
    ...
    wx.request({
      url: 'https://easy-mock.com/mock/59151e659aba4141cf220b1a/data/location',
      //data: {},
      method: 'GET',
      //header: {},
      success: (res) => {
      ...
      case 2: if(that.timer === "" || that.timer === undefined){
          ...
          wx.scanCode({
            success: (res) => {
              ...
              // var bike_name = res.result

              // if(device_list.name == bike_name.toString()){

              //   var dev_id = device_list.deviceId

              // }

              // // 利用扫码得到的Bike_ID，通过蓝牙与目标单车建立连接
              // wx.createBLEConnection({
              //   deviceId: dev_id.toString(),
              //   success: function(res) {
              ...
              // 请求服务器解锁密钥
              wx.request({
                url: 'https://easy-mock.com/mock/59151e659aba4141cf220b1a/data/key', 
                //data: {},
                method: 'GET', 
                success: function(res){
                  // 请求密钥成功隐藏等待框
                  ...
                  // 携带车号跳转到计费页
                  wx.redirectTo({
                    url: '../proceed/proceed?bike_id=' + res.data.data.bike_id + '&unlock_key=' + res.data.data.unlock_key,
                    success: function(res){
                      wx.showToast({
